...
from DsCML.models.build import build_model_2d, build_model_3d, build_2d_L2G, build_3d_L2G
...
def train(cfg, output_dir='', run_name=''):
    ...
    def cc(a,b):
        ...
        #     g_s_m = pd.Series(a[i].cpu().detach().numpy())
        #     g_a_d = pd.Series(b[i].cpu().detach().numpy())
        #     corr_gust = round(g_s_m.corr(g_a_d), 4)
        #     lst.append(corr_gust)
        return torch.tensor(cfg.TRAIN.XMUDA.lambda_cc * (1 - corr_gust) / 2).cuda()
        # return torch.tensor(cfg.TRAIN.XMUDA.lambda_cc * (1-np.mean(lst))/2).cuda()
    ...
    for iteration in range(start_iteration, max_iteration):
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_src > 0:
            ...
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'2D_3D_src_avg_2:',cc(F.log_softmax(seg_logit_2d_avg, dim=1),
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1)))
            ...
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'2D_3D_src_max_2:',cc(F.log_softmax(seg_logit_2d_max, dim=1),
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1)))
            ...
            #                           F.softmax(preds_2d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'3D_2D_src_point_2:',cc(F.log_softmax(seg_logit_3d_point, dim=1),
            #                           F.softmax(preds_2d_fe['seg_logit'].detach(), dim=1)))
        ...
        if cfg.TRAIN.XMUDA.lambda_xm_trg > 0:
            ...
            # seg_logit_2d_local = preds_2d_be['seg_logit3'] if cfg.MODEL_2D.DUAL_HEAD else preds_2d_fe['seg_logit']
            # seg_logit_3d_local = preds_3d_be['seg_logit3'] if cfg.MODEL_3D.DUAL_HEAD else preds_3d_fe['seg_logit']
            ...
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'2D_3D_trg_avg_2:',cc(F.log_softmax(seg_logit_2d_avg, dim=1),
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1)))
            ...
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'2D_3D_trg_max_2:',cc(F.log_softmax(seg_logit_2d_max, dim=1),
            #                           F.softmax(preds_3d_fe['seg_logit'].detach(), dim=1)))
            ...
            #                           F.softmax(preds_2d_fe['seg_logit'].detach(), dim=1),
            #                           reduction='none').sum(1).mean(),'3D_2D_trg_point_2:',cc(F.log_softmax(seg_logit_3d_point, dim=1),
            #                           F.softmax(preds_2d_fe['seg_logit'].detach(), dim=1)))
