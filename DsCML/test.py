...
from DsCML.models.build import build_model_2d, build_model_3d
from DsCML.data.build import build_dataloader
from DsCML.data.utils.validate import validate


def parse_args():
    parser = argparse.ArgumentParser(description='DsCML test')
    parser.add_argument(
        '--cfg',
        dest='config_file',
        default='',
...
def test(cfg, args, output_dir=''):
    logger = logging.getLogger('DsCML.test')
...
def main():
    ...
    logger = setup_logger('DsCML', output_dir, comment='test.{:s}'.format(run_name))
