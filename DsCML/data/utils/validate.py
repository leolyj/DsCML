...
from DsCML.data.utils.evaluate import Evaluator
...
def validate(cfg,
    ...
    with torch.no_grad():
        for iteration, data_batch in enumerate(dataloader):
            ...
            preds_2d_fe, out_2D_feature, img_indices = model_2d(data_batch)
            # preds_2d_be = LH_2d(out_2D_feature, img_indices)
            preds_3d_fe, out_3D_feature = model_3d(data_batch) if model_3d else None
            # preds_3d_be = LH_3d(out_3D_feature) if model_3d else None
            # preds_3d = model_3d(data_batch) if model_3d else None

            pred_label_voxel_2d = preds_2d_fe['seg_logit'].argmax(1).cpu().numpy()
            pred_label_voxel_3d = preds_3d_fe['seg_logit'].argmax(1).cpu().numpy() if model_3d else None

            # softmax average (ensembling)
            probs_2d = F.softmax(preds_2d_fe['seg_logit'], dim=1)
            probs_3d = F.softmax(preds_3d_fe['seg_logit'], dim=1) if model_3d else None
            ...
            seg_loss_2d = F.cross_entropy(preds_2d_fe['seg_logit'], data_batch['seg_label'])
            seg_loss_3d = F.cross_entropy(preds_3d_fe['seg_logit'], data_batch['seg_label']) if model_3d else None
